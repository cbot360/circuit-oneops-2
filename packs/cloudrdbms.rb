...
:monitors => {
    ...
    'DBprocess' => {:description => 'DBprocess',
        ...
        # there is a Linux process with name=mysqld  that we want to monitor.  check_process is a script that exists in all hcf VMs. To test/run it manually we can do: /opt/nagios/libexec/check_process.sh mysqld false mysqld
    ...
    'JavaAgentprocess' => {:description => 'JavaAgentprocess',
        :source => '',
        :chart => {'min' => '0', 'max' => '100', 'unit' => 'Percent'},
        # there is a Linux process with name=MySQLAgent  that we want to monitor.  check_process is a script that exists in all hcf VMs. To test/run it manually we can do: /opt/nagios/libexec/check_process.sh MySQLAgent false MySQLAgent
         ...
         "flow": [
                  {
                   ...
                   "direction": "from",
                   "targetClassName": "manifest.hcf.Cloudrdbms",
                   "flow": [
                            {
                             "relationName": "base.RealizedAs",
                             "execStrategy": "'+link[:strategy]+'",
                             "direction": "from",
                             "targetClassName": "bom.hcf.Cloudrdbms",
                             "actions": [
                                         {
